...
public alias InitVars 
	...
	rel[str cat, str dir] catDirs, list[ChangeSet] changesets];
...
public tuple[list[int], InitVars, MappingVars] cvsStats() {
    ...
    InitVars initVars = <cvsConfig.releases, cvsConfig.repo, 
	     cvsConfig.catDirs, transRecovered[1]>;
    
    MappingVars maps = getMappings(initVars, ());
    return <stats(initVars, maps), initVars, maps>;
}

public tuple[list[int], InitVars, MappingVars] cvsStats(rel[Tag, datetime]
	manualReleases) {
...
public list[ChangeSet] createChangesets(rel[Info committer, Resource
	resource, RevisionChange change, set[Tag] tags] changes) {
...
public set[WcResource] checkoutAndGetWcResources(Repository repo,
	CheckoutUnit cunit) {
...
public tuple[set[WcResource] cvs, set[WcResource] svn] getWcResources() {
    cvsRepo = cvs(fs("/export/scratch1/shabazi/CVS"),"software_20040603",
	    |file:///export/scratch1/shabazi/cvsWorkingCopy|,{});
    svnRepo = svn(ssh("svn+ssh://svn.cwi.nl", "waruzjan", "", 
	    |file:///ufs/shahbazi/.ssh/wshahbazian_rascal_rsa|), "", 
	    |file:///export/scratch1/shabazi/svnWorkingCopy|, {fileDetails()});
    wcResourcesCvs = checkoutAndGetWcResources(cvsRepo, 
	    cunit(createDateTime(2001,11,14,9,35,14,0,1,0)));
    wcResourcesSvn = checkoutAndGetWcResources(svnRepo, 
	    cunit(createDateTime(2001,11,14,9,35,14,0,1,0)));
...
public Resource searchResource(loc res, Resource root) {
	...
	childs = {r | Resource res <- range(locResources), 
	    folder(loc id, set[Resource] resources) := res, r <- resources};
    }
...
public map[loc m, loc e] getMissings(map[loc c, loc p] from, 
	map[loc c, loc p] to) {
...
public map[loc c, loc p] getChildParent(set[ChangeSet] changesets, bool s) {
    ...
    if (s) {
	svnBaseOriginLower = {<replaceAll(replaceFirst(replaceFirst(
		replaceFirst(toLowerCase(substring(rC.id.path, 40)), 
		"trunk/", ""), "branches/", ""), "tags/", ""), " ", "%20"), 
		rC> | rC <- svnRescs};
    } else {
...
public rel[bool res, int date, bool msg, int first, int second] 
	compareResources(rel[str author, str msg, ChangeSet cs] oneCs,
	rel[str author, str msg, ChangeSet cs] twoCs) {
...
public rel[bool res, int date, bool msg, int first, int second]
	compareResources(rel[datetime date, ChangeSet cs] oneCs, 
	list[datetime] oneDates, int fSize, rel[datetime date, ChangeSet cs] 
	twoCs, list[datetime] twoDates, int sSize, int max, datetime startMeUp, 
	datetime end) {
	...
	date = oneDates[i];
	i = i+1;
	if (!started && date >= startMeUp) {
	    print("Started!");
	    started = true;
	}
	if (started) {
	    rslt = compareResources(oneCs, oneDates, fSize, twoCs, 
		    twoDates, sSize, i, i+1);
	    result += rslt;
	    if (!rslt[0]) {
		rslt = compareResources(oneCs, oneDates, fSize, twoCs, 
			twoDates, sSize, i, i);
...
public rel[bool res, int date, bool msg, int first, int second]
	tstIt(rel[datetime date, ChangeSet cs] oneCs, list[datetime]
	oneDates, int fSize, rel[datetime date, ChangeSet cs] twoCs,
	list[datetime] twoDates, int sSize, int max, datetime startMeUp,
	datetime end) {

    printStartTimer("compareResources");
    reslt = compareResources(oneCs, oneDates, fSize, twoCs, twoDates, sSize, 
	    max,startMeUp,end);
    printStopTimer("compareResources");
    return reslt;
}

public rel[ChangeSet cvs, ChangeSet svn] compareResources(rel[str msg,
    datetime date, ChangeSet cs] fR, rel[str msg, datetime date, 
    ChangeSet cs] sR) {
...
public tuple[bool res, int date, bool msg] compareResources(ChangeSet fR,
	ChangeSet sR) {
...
public tuple[bool res, int date, bool msg, int first, int second]
	compareResources(rel[datetime date, ChangeSet cs] oneCs,
	list[datetime] oneDates, int fSize, rel[datetime date, ChangeSet cs] 
	twoCs, list[datetime] twoDates, int sSize, int first, int second) {
...
public tuple[rel[Info committer, Resource resource, RevisionChange change,
       set[Tag] tags], list[ChangeSet]] recoverChangesets(list[ChangeSet]
       resourceChanges, int maxSecondsDiff) {

    rel[Info committer, Resource resource, RevisionChange change, set[Tag] tags]
	    trans = recoverTransactions(resourceChanges, maxSecondsDiff);
...
public rel[Info committer, Resource resource, RevisionChange change,
    ...
    rel[str author, str message, datetime date, Resource resource,
	    ...
	    = {<rev[1].name ? "", rev[1].message ? "", rev[1].date, 
		cs.resource, rev[0], cs.revTags[rev[0].revision]> 
...
public tuple[map[int, list[int]] bench, InitVars initVars, 
       MappingVars mapVars] initStats(int amount) {
...
public tuple[list[Tag] releases, Repository repo, rel[str cat, str dir]
	catDirs] getCvsConfig() {
...
public tuple[list[Tag] releases, Repository repo, 
       rel[str cat, str dir] catDirs] getLinuxConfig() {
...
public tuple[list[Tag] releases, Repository repo, rel[str cat, str dir]
	catDirs, map[Tag version, CheckoutUnit cunit] manualReleases]
	getMetaEnvConfig() {
    ...
    catDirs += {<"aterm-java", "<rootDir>/<d>"> | d <- ["aterm-java", 
	    "shared-objects", "JJTraveler"]};
    catDirs += {<"toolbus", "<rootDir>/<d>"> | d <- ["toolbus", "toolbuslib"]};
    catDirs += {<"toolbus-ng", "<rootDir>/<d>"> | d <- ["toolbus-ng", 
	    "aterm-java"]};
    
    return <releases, repo, catDirs, manualReleases>;
}

public void setConfiguration(Repository repository, list[Tag]
	chosenReleases, map[Tag version, CheckoutUnit cunit] extraReleases,
	rel[str cat, str dir] categories) {
...
public map[int, list[int]] benchmark(InitVars iVars, MappingVars maps, 
	int amount) {
	...
	print("<i>) <s> --- results[i]");
	writeTextValueFile(
		|file:///export/scratch1/shabazi/benchmarkStats<now()>.txt|, 
		results); 
    }
    return results;
}
...
public list[int] stats(InitVars initVars, MappingVars maps) {
    ...
    //list[Tag] checkVersions = [label("1.0"), label("1.1"), label("1.2"), 
    //label("1.3"), label("1.4"), label("1.5"), label("1.5.1")];
    print("------------------STATS 1------------------");
    durations += statsOne(maps.tagChangeset, initVars.releases);
    print("------------------STATS 2------------------");
    durations += statsTwo(maps.versionRevisions, initVars.releases);
    print("------------------STATS 3------------------");
    durations += statsThree(initVars.repo, maps.tagChangeset, 
	    [lastVersion]/*initVars.releases*/);
    print("------------------STATS 4------------------");
    durations += statsFour(initVars.repo, maps.tagChangeset, 
	    [lastVersion]/*initVars.releases*/, initVars.catDirs);
    print("------------------STATS 5------------------");
    durations += statsFive(maps.versionNoMergesChangesets, domainMap, 
	    [lastVersion]/*initVars.releases*/);
...
public int statsFive(rel[Tag version, ChangeSet cs] versionChangesets,
	...
	result += calcDevelopers(version, "Author", 
		{<cs@author ? cs.committer, cs>
		| cs <- versionChangesets[version]}, false)<0,2,3,4>;
    }
    ...
    map[str mail, str company] emailCompany = (addr : domainMap[dom] 
	    | dom <- domain(domainMap), addr <- result.email, 
	    endsWith(addr, dom));
...
* 	the same user, even if alternative usernames are used For example,
* 	if the tuple <Linus Torvalds, linus@linux.com> exists in the input
* 	along with two other tuples: <Linus Torvalds, linux@linux.com> and
* 	<Torvalds, linux@linux.com>, then the resulted set will have the
...
public rel[Tag version, str action, str email, str devverName, ChangeSet cs] 
    calcDevelopers (Tag version, str action, rel[Info info, ChangeSet cs] 
    devvers, bool parseCsMessage) {
...
public tuple[tuple[list[Tag] releases, Repository repo, list[ChangeSet]
	changesets], tuple[rel[RevisionId child, RevisionId parent]
	childParents, map[Tag version, ChangeSet changeset] tagChangeset,
	map[RevisionId revId, ChangeSet cs] revChangeset, rel[Tag version,
	RevisionId revId] versionRevisions, rel[Tag version, ChangeSet cs]
...
public void setVars(tuple[list[Tag] releases, Repository repo,
	list[ChangeSet] changesets] initChanges) {
...
public void setVars(tuple[tuple[list[Tag] releases, Repository repo,
	list[ChangeSet] changesets],
...
public void setVars(
	tuple[list[Tag] releases, Repository repo, 
	list[ChangeSet] changesets] initChanges, tuple[rel[RevisionId child, 
	RevisionId parent] childParents,
	map[Tag version, ChangeSet changeset] tagChangeset,
	map[RevisionId revId, ChangeSet cs] revChangeset, 
	rel[Tag version, RevisionId revId] versionRevisions,
	rel[Tag version, ChangeSet cs] versionNoMergesChangesets] initMaps) {
...
public MappingVars getMappings(InitVars initVars, 
    ...
    rel[Tag version, ChangeSet cs] versionChangesets 
	    = versionRevisions o toRel(revChangeset);
...
public tuple[int total, rel[Tag version, RevisionId revision] versionRevisions] 
	getVersionRevisions (Repository repo, 
	rel[RevisionId child, RevisionId parent] childParents, 
	map[Tag version, ChangeSet cs] tagChangeset, 
	list[Tag] releases) {
...
*/
public rel[Tag version, RevisionId revision] getOnlyUniqueRevisions (
	rel[Tag version, RevisionId revision] versionRevisions, 
	list[Tag] releases) {
