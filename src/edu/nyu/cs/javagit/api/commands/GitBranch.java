...
/**
 * <code>GitBranch</code> provides an API for git-branch operation in a git repository.
 */
public final class GitBranch {
  ...
  public GitBranchResponse renameBranch(File repositoryPath, boolean forceRename, Ref oldName, 
    ...
    CheckUtilities.checkNullArgument(newName, "new name");
    CheckUtilities.validateArgumentRefType(oldName, Ref.RefType.BRANCH, "old name");
    CheckUtilities.validateArgumentRefType(newName, Ref.RefType.BRANCH, "new name");
  ...
  /**
   * Creates a branch. Allow the startPoint to be null to indicate to use the current working 
   * branch as the branch start point.
   *  
   * @param repositoryPath
   *          A <code>File</code> instance for the path to the repository. If null is passed, 
   *          a <code>NullPointerException</code> will be thrown.
   ...
   *              Thrown when there is an error executing git-branch.
   */
  public GitBranchResponse createBranch(File repositoryPath, Ref branchName, Ref startPoint) 
      throws IOException, JavaGitException {
    CheckUtilities.checkNullArgument(repositoryPath, "repository path");
    CheckUtilities.checkNullArgument(branchName, "branch name");
    CheckUtilities.checkNullArgument(startPoint, "start point");
    CheckUtilities.validateArgumentRefType(branchName, Ref.RefType.BRANCH, "branch name");
